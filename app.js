...
try{
	...
	var defaultPreference = {initcmd: '.', adminGroups: 'admins'};
...
var defaultGame = "with everyone's KolbICE";
...
*	it is set to loop, replay, or stoppedf
*/
...
bot.on('message', message => {
	...
	if(isCommand(message.content, 'setavatar')){
  	...
  	if(isCommand(message.content, 'loop') || isCommand(message.content, 'repeat')){
  		if(currentVoiceChannel !== message.member.voiceChannel){
			message.channel.send("Not in the bot's voice channel");
  			return;
  		}
